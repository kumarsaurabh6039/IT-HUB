...
export class SectionComponent implements OnInit {
  ...
  ngOnInit() {
    // We need more cards for a seamless scroll.
    // So we'll duplicate the array to make it longer.
    const videos1 = [...this.demoVideos];
    // A reversed copy for the second scroller
    const videos2 = [...this.demoVideos].slice().reverse();

    // Populating the scroller arrays
    this.scrollerVideos1 = [...videos1, ...videos1]; // Duplicate for a seamless loop
    this.scrollerVideos2 = [...videos2, ...videos2]; // Duplicate for a seamless loop
  }
  ...
  openModal(videoId: string) {
    // Construct the URL and mark it as safe
